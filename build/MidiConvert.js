	...
	var MidiConvert = {
	  ...
	  */
		deserialize: function deserialize(json) {
			return new _Midi.Midi().fromJSON(json);
		}
	};
	   ...
	   */
			...
			value: function fromJSON(json) {
				...
				json.tracks.forEach(function (track) {
					var newTrack = new _Track.Track(track.name).fromJSON(track);
					_this4.tracks.push(newTrack);
				});
			...
			value: function fromJSON(json) {
				...
				if (json.notes) {
					json.notes.forEach(function (note) {
						var newNote = new _Note.Note(note.midi, note.time).fromJSON(note);
